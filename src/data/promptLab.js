    ...
    prompts: [
      {
        ...
        prompt:
          '아래 문서를 검토해줘. 사실 오류·기술 오류·원문에 없는 추정을 찾아 "항목/문제/수정안/위험도" 표로 정리하고, 확신이 낮으면 "확인 필요"로 표기해줘.\n\n[여기에 검토할 문서 붙여넣기]',
      },
      {
        title: '데이터 품질 점검',
    ...
    prompts: [
      {
        ...
        prompt:
          '너는 건설기계 기업의 AX 전환 컨설턴트다. 내 직무는 [직무]이고 주요 업무는 [업무1, 업무2, 업무3]이다. 각 업무를 ① AI 적용 가능성(상/중/하), ② 기대 효과, ③ 추천 도구로 정리한 표를 만들고, 가장 먼저 시작할 업무 1가지를 근거와 함께 추천해줘.',
      },
      {
        title: '도구 선택 매트릭스',
        use: '업무 특성에 맞는 AI 도구 조합을 고른다.',
        prompt:
          '아래 업무들을 ChatGPT·Claude·Gemini·Genspark·Perplexity 중 어떤 도구가 가장 적합한지 "업무 / 추천 도구 / 이유 / 대안" 표로 정리해줘. 업무: [① 매뉴얼 번역 ② 시장 조사 ③ 발표자료 제작 ④ 장문 계약 검토 ⑤ 데이터 분석].',
      },
    ],
  },
  {
    ...
    prompts: [
      {
        ...
        prompt:
          '너는 [부서] 업무를 돕는 어시스턴트다. 다음 규칙을 지켜라: ① 업로드된 사내 자료에 근거해 답하고 근거가 없으면 "확인 필요"로 표기, ② 답변은 표/체크리스트로 구조화, ③ 안전·법규 문구는 임의로 바꾸지 않는다. 첫 인사로 어떤 정보를 주면 되는지 안내해줘.',
      },
      {
        title: '정비 이력 데이터 분석(Code Interpreter)',
        use: '엑셀/CSV를 올려 패턴을 분석한다.',
        prompt:
          '첨부한 정비 이력 데이터를 분석해줘. ① 부품별 고장 빈도 TOP10 막대그래프, ② 월별 정비비 추세선(이상치 표시), ③ 예방정비가 필요한 부품과 근거를 표로 제시해줘.',
      },
    ],
  },
  {
    ...
    prompts: [
      {
        ...
        prompt:
          '아래 수출 계약서를 검토해줘. 공급사(우리)에게 불리한 조항을 "조항 / 위험내용 / 위험도(상중하) / 수정 제안" 표로 정리하고, 각 항목에 원문 위치를 인용해줘. 확신이 낮으면 "법무 확인 필요"로 표기해줘.',
      },
      {
        title: '출처 인용 시장 보고서(Gemini Deep Research)',
        use: '근거가 붙은 조사 보고서를 만든다.',
        prompt:
          '동남아(인도네시아·베트남) 미니 굴삭기 시장을 조사해줘. 시장 규모·성장률, 주요 경쟁사, 수입 규제·인증, 유통 구조를 출처와 함께 정리하고 진출 리스크를 요약해줘.',
      },
    ],
  },
  {
  ...
  },
  {
    ...
    prompts: [
      ...
      },
      {
        ...
        prompt:
          '이 정비 작업 영상을 분석해 작업을 단계별 절차서로 변환해줘. 각 단계에 주의/안전 사항을 함께 적고, 필요한 공구 목록을 표로 정리해줘.',
      },
    ],
  },
  {
    ...
    prompts: [
      {
        ...
        prompt:
          '내 직무는 [기술문서/AS/영업/해외영업/마케팅/DX 중 택1]이다. 이 직무의 반복 업무 1개를 AI로 자동화하는 프로토타입을 설계해줘. ① 입력 → ② AI 처리 → ③ 사람 검수 → ④ 산출물 단계로 나누고, 각 단계의 도구·프롬프트·검수 기준을 표로 제시해줘.',
      },
    ],
  },
  {
    ...
    prompts: [
      {
        ...
        prompt:
          '너는 지식경영 전문가다. [정비/운전/품질 중 택1] 베테랑의 암묵지를 끌어내는 인터뷰 질문지를 만들어줘. 5W1H + Why-5 기법으로 15문항을 작성하고, 각 문항의 목적을 괄호로 표기해줘.',
      },
      {
        title: '1주일 데이터 수집 시스템 설계',
        use: '부서의 현장 데이터 수집 체계를 잡는다.',
        prompt:
          '내 부서 [부서명]의 1주일 현장 데이터 수집 시스템을 설계해줘. 요일별 수집 대상(사진·음성·문서)·도구·담당·보안등급을 표로 정리하고, 24시간 정리 룰을 반영해줘.',
      },
    ],
  },
  {
    ...
    prompts: [
      {
        ...
        prompt:
          '이 손글씨 점검표 이미지를 디지털 표로 변환해줘. 컬럼은 [점검일/점검자/항목/결과/특이사항]으로 하고, 판독이 불확실한 칸은 "[?]"로 표기해줘.',
      },
      {
    ...
    prompts: [
      {
        ...
        prompt:
          '내 부서 데이터에 적용할 메타데이터 표준 양식을 설계해줘. 15개 항목(제목·작성일·작성자·분류·보안등급 등)을 표로 정의하고, 각 항목의 입력 규칙과 예시를 함께 제시해줘.',
      },
      {
      ...
      },
      {
        ...
        prompt:
          '데이터 자산화 프로젝트의 ROI를 임원 보고용으로 정리해줘. 투입(시간·비용) 대비 효과(시간 절감·품질·재사용)를 정량화하고, 90일 로드맵(4단계)과 핵심 KPI를 표로 제시해줘.',
      },
    ],
  },
]
...
export const commonMistakes = [
  {
    ...
    fix: '너는 기술문서 작성자다. 30톤 굴삭기 일상점검 항목을 "점검부위/방법/주기" 표로 정리해줘.',
  },
  {
    bad: '좋은 마케팅 문구 써줘',
  ...
  },
  {
    ...
    fix: '공급사(우리)에게 불리한 조항만 찾아 "조항/위험/위험도/대안" 표로, 원문 위치를 인용해 정리해줘.',
  },
  {
    bad: '번역해줘',
    why: '용어·단위·안전문구 처리 기준이 없으면 현지에서 못 쓴다.',
    fix: '인도네시아어로 번역하되 용어집을 우선 적용하고 안전 경고문은 원형을 유지해줘.',
  },
]
  ...
  },
  {
    ...
    steps: [
      { instruction: 'ChatGPT에서 [GPT 만들기 → Configure]로 들어가, 아래 지시문을 Instructions에 붙여 넣으세요.',
        prompt: '너는 굴삭기 기술문서 어시스턴트다. 업로드된 매뉴얼에 근거해서만 답하고, 근거가 없으면 "매뉴얼 확인 필요"라고 답해줘. 모든 답변 끝에 근거 위치(섹션/페이지)를 표기해줘.',
  ...
  },
  {
    ...
    steps: [
      ...
      { instruction: '검토 관점을 명확히 준 아래 프롬프트를 입력하세요.',
        prompt: '아래 수출 계약서를 검토해줘. 공급사(우리)에게 불리한 조항을 "조항/위험내용/위험도(상중하)/수정 제안" 표로 정리하고, 각 항목에 원문 위치를 인용해줘. 확신이 낮으면 "법무 확인 필요"로 표기해줘.',
  ...
  },
  {
    ...
    steps: [
      ...
      { instruction: '조사 범위를 구체적으로 지정한 프롬프트를 입력하세요.',
        prompt: '동남아(인도네시아·베트남) 미니 굴삭기 시장을 조사해줘. 시장 규모·성장률, 주요 경쟁사, 수입 규제·인증, 유통 구조를 출처와 함께 정리하고 진출 리스크를 요약해줘.',
