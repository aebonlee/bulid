...
12. **제2권 전면 최적화(2026-07-15)** — 1권과 동일 기준(편집 차터 + 2026-07 버전 팩트시트)으로 PART 01~06·부록 재편집.
    - **구조 교정**: 빈 objectives 복구(PART 3~6 공통 결함), 캡션만 있고 img 없는 유령 figure 병합, 소견출 para→label 50여 건, 실습 exercise 정식화([실습 N-M] 번호 보존), 장식 구분선·이모지 제거, 그림·표·[코드 P-N] 일련화.
    - **부록(PART 08) 정비**: docx 파싱 잔재인 깨진 중복 섹션 2개(표가 셀 단위 para로 풀린 구버전) 제거, 부록 제목에 있었지만 실제로 없던 **용어집 39개 용어**(2권 본문 그라운딩)·**추천 학습 사이트 11개**(1권 부록에서 선별) 신설, 섹션 재정렬.
    - **버전 갱신**: GPT-4o 8건 → GPT-5.6 세대 등 팩트시트 기준 교체(API 비용 실측치는 "집필 시점 기준"으로 보존).
    - **강화**: promptbox 0→47개, tip 0→50개, PART 마무리 표준 5종, 분량 122k→209k자(1.7배). 저자 수치·사례·그림·실습 번호 전량 보존(검증 스크립트 통과).

13. **PPT 워크북(v0.5) 고유 콘텐츠 병합 + 인쇄본 생성기(2026-07-15)** — books/의 PPT 2종(237·245슬라이드)을 전수 추출·대조.
    - 분석 결과: 슬라이드의 90%+는 현행 교재의 구버전 판본(GPT-4o·DALL-E 등 구정보)이라 역병합 금지 판정. 고유 콘텐츠만 선별 — 1권 10건(직접 해보기 자기점검 규격, 자동화 도입 3단계+건너뛰기 금지, Copilot 환경 점검 5분 가이드, RPA 연동 구성도, LLM 토큰 예측 수치 예시, 검수 5원칙 '기록', 도메인 난이도 매트릭스, 결함 진단 산출물 예시표, 영상 분량 가이드), 2권 8건(수집 장비 확장 3 Phase+합의 경고, 암묵지 4채널+호손 효과, 라벨 스키마 워크북 MVL·UNCERTAIN·반례·IAA 파일럿, 음성 메모 5문장 골격, 5 Whys 감별진단→JSON 라벨 변환, Active Learning 등 효율화 3기법, PII 인벤토리 컬럼, DIKW 하향 번역).
    - 전 삽입은 "기존 블록 무손실 + 삽입만" 원칙, 부분수열 검사로 검증. 근거 슬라이드 추출본·분석 보고서는 세션 스크래치패드에 보존.
    - **인쇄본 생성기**: content.json 1권 → 워드(.docx, python-docx)·한글(.hwpx, python-hwpx — 패키지 검증·재열기·편집기 안전성 게이트 통과) 생성. 산출물은 books/(gitignore) 하위.
